...
def addLines(img, lines, imgname='adding_lines', color=(0, 0, 255), rand_color=False, thickness=1,
             display_=False):
    newim = np.copy(img)
...
def addPoints(img, points, imgname='adding_points', color=(0, 255, 255), thickness=2,
              display_=False):
    newim = np.copy(img)
    for x, y in points:
        cv2.ellipse(newim, (int(x), int(y)), (thickness, thickness), 0, 0, 360, color, -1)
    if display_:
        showIm(imgname, newim)
    return newim


def showJunctionPrediction(img, junctions, thetas, color=(0, 255, 0), imgname='junction',
                           display_=False, thickness=2, rand_color=False):
    newim = np.copy(img)
    ...
    for (x, y), theta in zip(junctions, thetas):
        ...
        for t in theta:
            if isinstance(t, tuple):
                ...
                z = t
            x1, y1 = (x + 12.5 * cos(z * pi / 180.), y + 12.5 * sin(z * pi / 180.))
            cv2.line(newim, (int(x), int(y)), (int(x1), int(y1)), l_color, thickness)
            cv2.line(newim, (int(x), int(y)), (int(x1), int(y1)), l_color, thickness)
    if display_:
        showIm(imgname, newim)
...
def generate_html(imglst, folder, table_keys=['gt', 'lsd'], img_name=True):
    possible_keys = {'gt': "groudtruth junction", "lsd": "LSD, IPOL 2012, TPAMI 2010."}
