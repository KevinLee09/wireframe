...
def init_optimizer(opt, model):
    if opt.optimizer == 'sgd':
        optimizer = torch.optim.SGD(model.parameters(), lr=opt.lr, momentum=0.9, weight_decay=1e-6)
    elif opt.optimizer == 'adam':
        optimizer = torch.optim.Adam(model.parameters(), lr=opt.lr, betas=(opt.momentum, 0.999),
                                     eps=1e-8, weight_decay=opt.weightDecay)
    elif opt.optimizer == 'rms':
        optimizer = torch.optim.RMSprop(model.parameters(), alpha=0.9, lr=opt.lr, eps=1e-6,
                                        weight_decay=1e-6)
    return optimizer
...
def init_dataloader(opt):
    test_loader = torch.utils.data.DataLoader(
        init_dataset(opt.hype, 'test'), batch_size=opt.batch_size, shuffle=False,
        num_workers=1) if opt.test else None

    train_loader = torch.utils.data.DataLoader(
        init_dataset(opt.hype, opt.split), batch_size=opt.batch_size, shuffle=True,
        num_workers=max(4, opt.batch_size), pin_memory=True) if not opt.test else None
...
def main():
    ...
    net = opt.net
    opt.cuda = True  # use cuda by default
    ...
    if not opt.test:
        ...
        checkpoint = ref.output_root / "{expID}/model_{epoch_test}.pth".format(
            expID=opt.exp, epoch_test=epoch_test)
