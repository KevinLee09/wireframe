...
class Plotter(object):
    ...
    def init_plot_param(self, H):
        ...
        self.centers = np.array([i * bin_width for i in range(self.num_bin)], dtype=np.float32)
        self.junc_anchors = generate_anchors((self.grid_size, self.grid_size),
                                             stride=self.stride).reshape((-1, 2))
    ...
    def plot_junction_simple(self, image, output_result, junc_thresh=0.5, theta_thresh=0.5,
                             plot_=True, size_info=None, keep_conf=False):
        ...
        for idx, (jx, jy) in enumerate(junctions):
            ...
            for th in nthetas[idx]:
                ...
                xe, ye = (jx + self.line_length * dx, jy + self.line_length * dy)
